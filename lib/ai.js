...
async function generateImagePrompt(quote) {
  // Extract the character name and anime from the quote attribution
  ...
  const system =
    'You are a visual scene designer for anime/manga art. ' +
    ...
    'Your scenes should be BRIGHT, CLEAN, UPLIFTING, and BEAUTIFUL — never dark, creepy, or horrifying. ' +
    'Even sad or melancholy quotes should have a gentle, poetic beauty to them.';

  const user =
    `Anime quote: "${quote}"\n` +
    (charName ? `Character: ${charName}\n` : '') +
    (animeName ? `From: ${animeName}\n` : '') +
    '\nDescribe a beautiful, bright anime scene that captures this quote\'s emotion. ' +
    (charName ? `Feature ${charName} recognizably (their signature look/outfit from ${animeName || 'the anime'}). ` : '') +
    'Include: character pose/expression, a fitting environment, soft/warm lighting. ' +
    'IMPORTANT: The scene must feel warm, hopeful, or gently emotional — NEVER creepy, dark, grim, or horror-like. ' +
    'Keep it 15-25 words. Optimize for clean black & white line art with good contrast.';
...
async function generateImage(prompt, style = 'anime') {
  ...
  const styleMap = {
    anime:
      'Beautiful anime/manga style illustration for e-ink display. ' +
      'Clean bold lines, clear shapes, bright and warm mood, soft shading, no text overlays. ' +
      'Black and white only. The art should feel uplifting, gentle, and visually appealing — NEVER dark, creepy, or horror-themed. ' +
      'Wide cinematic 5:2 composition.',
    realistic:
