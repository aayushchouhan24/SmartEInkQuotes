...
function thresholdToBitmap(grayBuf, w, h, threshold = 160) {
  const rowBytes = Math.ceil(w / 8);
  const out = new Uint8Array(rowBytes * h);

  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const v = grayBuf[y * w + x];
      if (v < threshold) {
        out[y * rowBytes + Math.floor(x / 8)] |= 0x80 >> (x % 8);
      }
    }
...
async function imageToBitmap(imageBuf) {
  const raw = await sharp(imageBuf)
    ...
    .normalise()
    .linear(1.18, -12)
    .sharpen({ sigma: 1.2, m1: 0.6, m2: 1.1 })
    .median(1)
    .raw()
    .toBuffer();

  return thresholdToBitmap(raw, DISPLAY_W, DISPLAY_H, 162);
}
